...
Adafruit_Simple_AHRS::Adafruit_Simple_AHRS(Adafruit_Sensor* accelerometer, Adafruit_Sensor* gyro, bool isGyro):
...
bool Adafruit_Simple_AHRS::getRollPitchYaw(sensors_vec_t* orientation) {
  // Validate input and available sensors.
  if (orientation == NULL || _accel == NULL || _gyro == NULL) return false;

  // Grab an acceleromter and gyro reading.
  sensors_event_t accel_event;
  _accel->getEvent(&accel_event);
